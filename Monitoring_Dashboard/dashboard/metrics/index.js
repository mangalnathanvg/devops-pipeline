...
try
{
	ip = fs.readFileSync('/bakerx/dashboard/metrics/ip.txt').toString();
	// fs.readFile('/bakerx/dashboard/cloud_ip_details.json', 'utf8', (err, jsonString) => {
	// 	if (err) {
	// 		console.log("Error in reading the json file.", err);
	// 		return
	// 	}
	// 	try{
	// 		const ip_details = JSON.parse(jsonString);
	// 		ip = ip_details.monitor;
	// 	} catch (err){
...
{
	console.log(e);
	throw new Error("Missing required text file");	
}
